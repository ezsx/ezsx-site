  ...
  const routes = [
    {
      ...
      absent: [
        ...
        /Backend совместного холста в реальном времени для живого мероприятия/,
    ...
    },
    {
      ...
      phrases: [
        ...
        /Надёжно запустить полный поиск seed на двух GPU/,
        /project brief \/ 30 second read/,
        /Унаследованный CUDA pipeline не запускался стабильно/,
        /idempotent PostgreSQL job/,
        /фактическая точность составила 0\.898/,
        /self-repo benchmark показал 83\.3% recall @ 10/,
        /На нагрузочном тесте система выдержала 1,5-2 тыс\./,
        /expand story/,
        /Inspect the CUDA worker/,
        /expand profiler/,
        /One warp through the search/,
        /pause trace/,
        /Выдавать защищённые подключения и сохранять контроль над парком узлов/,
