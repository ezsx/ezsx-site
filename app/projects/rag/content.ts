  ...
  ru: {
    ...
    brief: {
      ...
      points: [
        {
          label: "challenge",
          text: "Retrieval должен находить и точные термины, и смысловые совпадения, не позволяя модели отвечать без достаточных источников.",
        },
        {
          label: "built",
          text: "Я объединил ingestion, ReAct planning, exact и semantic retrieval, reranking, проверку источников и локальный inference в Windows, WSL2 и Docker.",
        },
        {
          label: "result",
          text: "На выборке из 120 проверенных кейсов фактическая точность составила 0.898 для 105 вопросов с ответом, поддержка ответа источниками получила оценку 0.886 на 65 retrieval-кейсах, а все 15 запросов, требующих отказа, обработаны корректно.",
        },
      ],
    },
    navLabel: "RAG evidence system story",
    stages: [
      ...
      },
      {
        ...
        summary:
          "Агент определяет, нужен ли retrieval, аналитика, навигация или безопасный отказ.",
        detail:
      ...
      },
      {
        ...
        detail:
          "Каждый подзапрос начинает со 100 BM25 и 40 dense-кандидатов. Weighted RRF отдаёт лексическому пути вес 3:1, ColBERT выполняет late-interaction MaxSim, а результаты объединяются MMR-проходом с балансом релевантности и разнообразия.",
        tags: ["BM25 100 · dense 40", "RRF 3:1", "ColBERT 128-d MaxSim"],
      },
      {
        id: "ground",
        ...
        summary:
          "Cross-encoder уточняет порядок, а независимый semantic guard может сохранить полезный документ.",
        detail:
          "CE scores управляют reranking, поиском разрывов и адаптивным отсечением; dense rank защищает сильных кандидатов от ошибочного cutoff. Затем пункты из query plan проверяются по оставшимся документам. В текущем coverage path оценка ниже 0.75 запускает один точечный retrieval по недостающим аспектам.",
        tags: ["CE + cosine guard", "coverage ≥ 0.75", "1 targeted refinement"],
      },
      {
        id: "answer",
        ...
        detail:
          "FastAPI стримит ход работы, наблюдения, citations и финальный ответ как typed SSE events. Ответ опирается на выбранные посты, а evaluation раздельно измеряет фактическую точность, поддержку ответа источниками, отказы и доверительные интервалы.",
        tags: ["Qwen3.5-35B-A3B", "typed SSE events", "numbered citations"],
      },
    ],
    headlines: [
